angular.module('app.directives').directive('scoreCard', ['$timeout', function ($timeout) {
    return {
        restrict: 'E',
        scope: {
            game: '='
        ...
        link: function (scope, element, attrs) {
            ...
            if (scope.game && scope.game.gameHoles && scope.game.gameHoles.length > 0) {
                initData();
            }

            scope.$on('SuperGameChanged', function (evt, data) {
                scope.game = data;
                if (scope.game && scope.game.gameHoles && scope.game.gameHoles.length > 0) {
                    initData();
                    $timeout(function () {
